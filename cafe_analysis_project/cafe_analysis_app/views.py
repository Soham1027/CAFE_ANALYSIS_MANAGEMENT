...
def dashboard_view(request):
    ...
    age_data = PersonDetection.objects.filter(detection_time__gte=start_date).values('age').annotate(count=Count('age')).order_by('age')

    # Data for gender distribution (bar chart)
    gender_data = PersonDetection.objects.filter(detection_time__gte=start_date).values('gender').annotate(count=Count('gender')).order_by('gender')

    # Data for average time spent (line chart)
    avg_time_spent_data = PersonDetection.objects.filter(detection_time__gte=start_date).aggregate(avg_time_spent=Avg('time_spent'))
